...
<p>Although our first game is complete and ready to be tested using an outside emulator, it will be polished over the course of the next releases (with the possibility of new features). A very basic prototype of Space Invaders has been created, which showcases player movement and the sprites that will be featured in the final game (very early stages of development). Both of these games can be tested using Octo. Since our second game is expected to be much more complex than the first, we will also be creating a progress and design document specifically for that game.</p>
...
<p>We will continue to work through our problems as we head towards Release 3. Now that midterms are over, all group members have more time to work on their respective tasks. Additionally, every group member is required to specify when they will not be able to complete their tasks ahead of time so that the other members aren't put under a stressful situation and can plan accordingly.</p>
