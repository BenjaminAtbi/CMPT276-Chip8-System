...
- all README files have been updated to include new information
  ...
  -  General README includes directions on how to test program loading, as well as information on the new website prototype
...
<p>In Release 3, we only managed to implement one new usable feature which is program loading via plaintext opcodes in a .txt file. To test this feature, upload the file named "test.txt" from the emulator directory. If the test is passed, a sprite will appear on the emulator screen. Otherwise, nothing will be displayed. Our new website, while incomplete, can be tested as detailed in the general project README file. The prototype is mostly there to show a template that our final website will follow.</p>
